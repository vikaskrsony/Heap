  ...
  class Solution
{
    ...
    {
        long totalSumOfRopes = 0;
        PriorityQueue<Long> ropes = new PriorityQueue<>();
        for(long rope : arr){
            ropes.add(rope);
        }
        while(ropes.size() > 1){
            long firstRope = ropes.poll();
            long secondRope = ropes.poll();
            long sumOfRopes = firstRope + secondRope;
            totalSumOfRopes += sumOfRopes;
            ropes.add(sumOfRopes);
        }
        return totalSumOfRopes;
    }
}
